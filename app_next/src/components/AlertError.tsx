...
var AlertError = (props: TitleProps) =>{
    ...
    const [messageAlert, setMessageAlert] = useState(<div/>)    
    ...
    useEffect(()=>{
        ...
        if(email=='aurieljames11@gmail.com'){
            setMessageAlert(messageSuccess)
        }
        else if(email==''){
            setMessageAlert(messageNull)
        }
        else{
    ...
    const messageError =
        ...
        <div className="ml-3 text-sm font-normal text-meta-1">Log in Unsuccessful</div>
        </div>
